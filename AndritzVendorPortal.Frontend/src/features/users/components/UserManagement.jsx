...
function UserDetailModal({ user, onClose, onUpdated, onDeleted }) {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true)
    try {
      const data = await usersService.update(user.id, {
...
export default function UserManagement() {
  ...
  const handleUserUpdated = (updated) => {
    ...
    if (currentUser && updated.id === currentUser.id) {
      updateAuthUser({ name: updated.fullName, fullName: updated.fullName })
    }
  }

  const handleUserDeleted = (id) => {
    const deleted = applyDeleted(id)
    if (deleted) {
  ...
  const handleRestore = async (u) => {
    try {
      const restored = await restoreUser(u.id)
