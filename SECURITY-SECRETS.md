...
| Secret | Was | Rotate by |
| --- | --- | --- |
| SMTP relay password | `EmailSettings:Password`, appsettings.json lines 19–27 | Ask IT to reset the password for the `SCMVendoApprovalNotification` mailbox |
| Seed admin password | `Seed:DefaultAdminPassword` = `Andritz@1234` | Choose a new one; change the existing admin account's password in the portal too, since the seeded one is live |
...
| SQL login | app connects as **`sa`** | Create the least-privilege `sot_app` login (below) and change the `sa` password |
